...
STRATEGY_NAME: str = "revert_median"

# --- Strategy settings (edit here) ---

# Trade thesis (printed to strategy/strategy_output.txt when run via the strategy loader).
TRADE_THESIS: str = (
    "Core thesis: In a chosen “liquid enough” universe, 24h winners tend to mean‑revert and 24h losers tend to bounce. "
    "So it shorts the strongest 24h performers and longs the weakest 24h performers (within the selected universe)."
)
...
DEFAULT_MAX_OI_SKEW: Optional[float] = 0.75
...
def pick_tickers(
    *,
    ...
    universe = filtered
    median_24h_chg_pct, longs, shorts = _revert_split_by_24h_change(universe)
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Strategy: revert median (mean-reversion split by 24h change).")
    ap.add_argument(
        "--json-path",
...
def main() -> int:
    ...
    global DEFAULT_TOP_SPEC, DEFAULT_EXCLUDE_CSV, DEFAULT_MAX_OI_SKEW, DEFAULT_MIN_VOL_24H
