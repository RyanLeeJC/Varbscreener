...
def _repo_root_from_here() -> str:
    # This file lives in: <repo_root>/Varibot/sideways_median_filter.py
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Sideways median split: top N by OI (ex BTC/ETH, blacklist, ex OI Skew>0.95), "
        "then long bottom half by 24h change and short top half."
    )
    ap.add_argument(
        "--json-path",
...
def main() -> int:
            ...
            json.dumps(
                {
                    ...
                    "long": res.underperformers,
                    "short": res.outperformers,
                },
                indent=2,
            )
        )
    ...
    print()
    print(f"Bottom half by 24h chg (LONG) [{len(res.underperformers)}]:")
    print(",".join(res.underperformers))
    print()
    print(f"Top half by 24h chg (SHORT) [{len(res.outperformers)}]:")
